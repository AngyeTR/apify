...
export const ProductList =()=>{
    ...
    useEffect(()=>{
        getByCompanyId("Products", 1).then(res=> setProducts(res.data))
...
{     productsToShow?.map((product) => <CardProduct data={product}/>)}        
{     productsToShow?.length == 0 && <div ><Text> No se encontraron Productos</Text></div>}        
    </div>
  </div>
</div>
    )
