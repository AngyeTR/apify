...
export const adaptWarehouseModel = (dataSet, origin, selectedCity,) =>{
   !dataSet.createdBy && (dataSet["createdBy"] = stored?.useremail)
   dataSet.city && delete dataSet.city
   dataSet["modifiedBy"]= stored?.useremail
...
export  const adaptProductModel =  (dataSet, origin, stock) => {
    ...
    const adaptedImages = async (images) => {
      console.log(images)
        ...
        //         dataSet["images"].push({url: url,  modifiedBy: stored?.useremail, createdBy: stored?.useremail })
        ...
        for(let i = 0; i < images.length;  i++){
          try {
            ...
            const url = await postImage({name: `productImage${Date.now()}`, "base64": base64, "imageType": 2}).then(res => {return res.data})
          dataSet["images"][i] = {url: url,  modifiedBy: stored?.useremail, createdBy: stored?.useremail }
...
export const adaptDelegateModel = (idUser)=> {
  return  {
    ...
    createdBy:	stored?.useremail,
    modifiedBy:	stored?.useremail,
    idCompany: stored?.usercompany.id,
    idUser: idUser,
